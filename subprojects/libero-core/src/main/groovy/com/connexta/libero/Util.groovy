...
def static printConfig(Options options, Config config) {
    ...
    println "Commit prefix: ${config.commitPrefix}"
    ...
    println "Maven Local Repo: ${config.localRepo}"
    println "Maven Release Repo: ${config.mavenRepo}"
    println "Maven Settings: ${config.mavenSettings}"
    println "Cleaning artifacts from local repo matching: ${config.cleanArtifacts}"
    println "Activating profiles: ${config.profiles}"
